...
%   perform the BB-method on this step + line search       
    GradD_ = zeros(n,m);
    for i = 1:p
        GradD_ = GradD_ + M(:,:,i)*(D_*X(:,i)-Y(:,i))*X(:,i)';
    end
    GradD = zeros(n,m);
    for i = 1:p
        GradD = GradD + M(:,:,i)*(D*X(:,i)-Y(:,i))*X(:,i)';
    ...
    alpha_D = trace(s_d'*s_d)/trace(s_d'*s_gd);
    
    % non-monotone line search
    stopping = 0;
    GradDsq = trace(GradD'*GradD);
    value = evaluation(Y, D+alpha_D*GradD, X, Corr, gamma);
    ...
    D_ = D; D = D+alpha_D*GradD; % D_ is to record, D is the current.
    nrmD = sqrt(sum(D.^2,1));
    id = nrmD>1; s = sum(id);
    if s>0     
        D(:,id) = D(:,id)*spdiags(nrmD(id)'.^(-1),0,s,s);
