function [img_lasso, time_lasso] = inpainting_triple_lasso(img_corr, corr, para)
...
overlap = blsize/2;
% img_ormp = zeros(N,M); 
img_lasso = zeros(N,M, num); S = img_lasso(:,:,1);
...
for i = 0:row-1
    for j = 0:col-1
        ...
        [patch_lasso, time_lasso] = inpainting_double(patch,corruption, para);
    	...
    	for k = 1:num
            img_lasso([1:blsize]+i*(blsize-overlap), [1:blsize]+j*(blsize-overlap),k) = img_lasso([1:blsize]+i*(blsize-overlap), [1:blsize]+j*(blsize-overlap),k)+patch_lasso(:,:,k);
        end
...
function [img_lasso, time_lasso] = inpainting_double(img_corr, corr, para)
...
for k = 1:n_patch
    ...
    recovered_ormp = Dl*X_ormp; 
    imgpatch_ormp(:,:,k) = patch2img_copy(recovered_ormp, blsize2, overlap2, [blsize1,blsize1]);

    num = size(para.lasso,2); 
    X_lasso = zeros(m,p,num); time_lasso = zeros(1,num);
    for i = 1:num
        tt = tic;
        for l = 1:p
            X_lasso(:,l,i) = lasso(diag(Corr(:,l))*Dl, Y(:,l), 'lambda', para.lasso(i));
        end
        time_lasso(i) = toc(tt);
        recovered_lasso(:,:,i) = Dl*X_lasso(:,:,i);
        imgpatch_lasso(:,:,k,i) = patch2img_copy(recovered_lasso(:,:,i), blsize2, overlap2, [blsize1,blsize1]);
    end
...
S = zeros(N,M);
count = 0;
img_ormp = zeros(N,M); img_lasso = zeros(N,M,num);
for k = 0:n_row-1
    for j = 0:n_col-1
        count = count+1;
        ...
        for i = 1:num
            img_lasso(row_patch, col_patch, i)=img_lasso(row_patch, col_patch, i)+imgpatch_lasso(:,:,count,i);
        end
