function I = patch2img(X, Corr, blsize, overlap, im_size)
...
% Corr: (blsize^2 x p) matrix containing the vectorised mask. 
% Each column is the mask of the corresponding column of X
% blsize: patch size
...
for i = 0:row-1
    for j = 0:col-1
        ...
        current_corr = reshape(Corr(:, counter), blsize, blsize);
        % Update cumulative number of covers
        S(x_patch, y_patch) = S(x_patch, y_patch) + (~current_corr);
        % Update patch
        I(x_patch, y_patch) = I(x_patch, y_patch) + current_patch.*(~current_corr);
    end
end
